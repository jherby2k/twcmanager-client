...
class TWCManagerClient:
    ...
    async def async_get_config(self) -> dict[str, Any]:
        ...
        return await self._async_get("http://" + self._host + ':8080/api/getConfig')
    

    async def async_get_policy(self) -> dict[str, Any]:
        """Get the policy configuration."""
        return await self._async_get("http://" + self._host + ':8080/api/getPolicy')


    async def async_get_slave_twcs(self) -> dict[str, Any]:
        """Get a list of connected slave TWCs and their state."""
        return await self._async_get("http://" + self._host + ':8080/api/getSlaveTWCs')


    async def async_get_status(self) -> dict[str, Any]:
        """Get the current status (charge rate, policy)."""
        return await self._async_get("http://" + self._host + ':8080/api/getStatus')
